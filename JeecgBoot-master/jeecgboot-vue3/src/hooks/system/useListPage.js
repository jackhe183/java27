...
export function useListPage(options) {
     ...
     */
    function doRequest(api, options) {
 ...
 */
export function useListTable(tableProps) {
    // 自适应列配置
     ...
     * @param formConfig
     */
    function setTableProps(formConfig) {
        const replaceAttributeArray = ['baseColProps', 'labelCol'];
        for (let item of replaceAttributeArray) {
            if (formConfig && formConfig[item]) {
                if (defaultTableProps.formConfig) {
                    let defaultFormConfig = defaultTableProps.formConfig;
...
//# sourceMappingURL=useListPage.js.map