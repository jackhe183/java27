 ...
 */
export function registerComponent(type, component, spanComponent) {
 ...
 * @param promise
 */
export async function registerAsyncComponent(type, promise) {
    const result = await promise;
    if (isFunction(result.installJVxe)) {
        result.install((component, spanComponent) => {
            addComponent(type, component, spanComponent);
            registerOneComponent(type);
        });
    }
 ...
 * @param promise
 */
export function registerASyncComponentReal(type, component) {
 ...
 * @param type 组件 type
 */
export function registerOneComponent(type) {
...
/** 注册可编辑组件 */
function createEditRender(type, component, spanComponent) {
    // 获取当前组件的增强
...
/** 注册普通组件 */
function createCellRender(type, component = componentMap.get(JVxeTypes.normal)) {
    // 获取当前组件的增强
    const enhanced = getEnhanced(type);
    VXETable.renderer.add(JVxeTypePrefix + type, {
        // 默认显示模板
