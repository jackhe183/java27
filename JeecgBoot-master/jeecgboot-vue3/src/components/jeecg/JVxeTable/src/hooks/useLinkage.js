 ...
 * 多级联动
 */
export function useLinkage(props, data, methods) {
    // 整理多级联动配置
    watch(() => props.linkageConfig, (linkageConfig) => {
    ...
    // 获取联动的key顺序
    function getLinkageKeys(key, keys) {
        let col = props.columns?.find((col) => col.key === key);
        if (col) {
            keys.push(col.key);
            // 寻找下级
            if (col.linkageKey) {
...
//# sourceMappingURL=useLinkage.js.map