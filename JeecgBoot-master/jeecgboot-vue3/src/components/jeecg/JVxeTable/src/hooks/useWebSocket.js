...
const vs = {
    ...
    onVM: {
        /** 收到更新表格的消息 */
        onUpdateTable({ props, data, methods }, row, caseId) {
...
export function useWebSocket(props, data, methods) {
    const args = { props, data, methods };
    watch(() => props.socketReload, (socketReload) => {
        if (socketReload) {
            vs.initialWebSocket();
