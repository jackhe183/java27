...
export function useJVxeComponent(props) {
    ...
    function parentTrigger(name, event, args = []) {
        args.unshift(packageEvent(name, event));
        trigger('trigger', name, args);
    }
    function packageEvent(name, event = {}) {
        event.row = row.value;
 ...
 * 获取组件默认增强
 */
export function useDefaultEnhanced() {
    return {
        installOptions: {
            autofocus: '',
        },
...
//# sourceMappingURL=useJVxeComponent.js.map