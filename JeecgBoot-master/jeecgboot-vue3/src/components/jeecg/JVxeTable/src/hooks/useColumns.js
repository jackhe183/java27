...
export function useColumns(props, data, methods, slots) {
    data.vxeColumns = computed(() => {
        let columns = [];
        if (isArray(props.columns)) {
            // handle 方法参数
            const args = { props, slots, data, methods, columns };
            let seqColumn, selectionColumn, expandColumn, dragSortColumn;
            const handleColumn = (column, container) => {
                ...
                if (Array.isArray(col.children) && col.children.length > 0) {
                    const children = [];
                    col.children.forEach((child) => handleColumn(child, children));
            ...
            props.columns.forEach((column) => handleColumn(column, columns));
...
/** 处理内置列 */
function handleInnerColumn(args, col, handler, assign) {
 ...
 * 处理隐藏列
 */
function handleHiddenColumn({ col, columns }) {
 ...
 * 处理行号列
 */
function handleSeqColumn({ props, col, columns }) {
    // 判断是否开启了行号列
    if (props.rowNumber) {
        let column = {
            type: 'seq',
 ...
 * 处理可选择列
 */
function handleSelectionColumn({ props, data, col, columns }) {
    // 判断是否开启了可选择行
 ...
 * 处理可展开行
 */
function handleExpandColumn({ props, data, col, columns }) {
    // 是否可展开行
    if (props.rowExpand) {
        let width = 40;
...
/** 处理可排序列 */
function handleDragSortColumn({ props, data, col, columns, renderOptions }) {
    // 是否可拖动排序
    if (props.dragSort) {
        let width = 40;
...
function handlerCol(args) {
    ...
    // 渲染选项
    let $renderOptions = { name: JVxeTypePrefix + type };
 ...
 * 处理字典
 */
async function handleDict({ col, methods }) {
        ...
        try {
            ...
            if (!isPromise(col.params.optionsPromise)) {
                col.params.optionsPromise = new Promise(async (resolve) => {
                    ...
                    const dictOptions = await initDictOptions(dictCodeString);
 ...
 * 处理插槽
 */
function handleSlots({ slots, col, renderOptions }) {
    // slot 组件特殊处理
...
/** 处理联动列 */
function handleLinkage({ data, col, renderOptions, methods }) {
...
function handleReloadEffect({ props, data, renderOptions }) {
