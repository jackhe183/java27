...
export function usePopBiz(ob, tableRef) {
    ...
    // 处理动态参数和系统变量
    function handleCodeParams(onlyCode = false) {
        if (!props.code) {
            return { code: '', query: '' };
        }
     ...
     * 处理求和的列 合计逻辑 [待优化 3.0]
     */
    function handleSumColumn(metaColumnList, dataTotal) {
        // 获取需要合计列的dataIndex
     ...
     * @param columns
     */
    function getNeedSumColumns(columns) {
        let arr = [];
     ...
     * 处理列的href和字典翻译
     */
    function handleColumnHrefAndDict(columns, fieldHrefSlotKeysMap) {
        for (let column of columns) {
     ...
     * @param columns
     */
    function handleGroupTitle(columns) {
        let newColumns = [];
        for (let column of columns) {
            //排序字段受控  ---- 此逻辑为新增逻辑 待
...
//# sourceMappingURL=usePopBiz.js.map