...
export function useSelectBiz(getList, props, emit) {
    //接收下拉框选项
    const selectOptions = inject('selectOptions', ref([]));
    //接收已选择的值
    const selectValues = inject('selectValues', reactive({ value: [], change: false }));
    // 是否正在加载回显
    const loadingEcho = inject('loadingEcho', ref(false));
    //数据集
    ...
    async function onSelectChange(selectedRowKeys, selectRow) {
...
//# sourceMappingURL=useSelectBiz.js.map