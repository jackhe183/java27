 ...
 * 自定义选择列
 */
export function useCustomSelection(propsRef, emit, wrapRef, getPaginationRef, tableData, childrenColumnName) {
    ...
    watch(() => unref(propsRef)?.rowSelection?.selectedRowKeys, (val) => {
    ...
    */
    ...
    // 选择全部
    function onSelectAll(checked, flag = 'currentPage') {
    ...
    // 分批全选
    function batchesSelectAll(hidden, checked, minSelect) {
    ...
    function onSelectParent(record, checked) {
        if (unref(propsRef)?.isTreeTable && unref(propsRef)?.rowSelection?.checkStrictly === false && !isRadio.value) {
            let condition = true, currentRecord = record;
            while (condition) {
                const parentRecord = findParent(tableData.value, currentRecord, childrenColumnName.value);
                if (parentRecord) {
                    const childrenRecordKeys = [];
    ...
    // 用于判断是否是自定义选择列
    function isCustomSelection(column) {
     ...
     */
    function recordIsShow(index, threshold = 3) {
    ...
    function renderCheckboxComponent({ record }) {
        ...
        return (<Checkbox {...checkboxProps} key={'j-select__' + recordKey} checked={selectedKeys.value.includes(recordKey)} onUpdate:checked={(checked) => onSelect(record, checked)} 
    ...
    function renderRadioComponent({ record }) {
        ...
        return (<Radio {...checkboxProps} key={'j-select__' + recordKey} checked={selectedKeys.value.includes(recordKey)} onUpdate:checked={(checked) => onSelect(record, checked)} 
        // update-begin--author:liaozhiyang---date:20230326---for：【QQYUN-8694】BasicTable在使用clickToRowSelect=true下，selection-change 事件在触发多次
        onClick={(e) => e.stopPropagation()}/>);
    }
    // 创建选择列
    function handleCustomSelectColumn(columns) {
    ...
    function setSelectedRowKeys(rowKeys) {
        ...
        const allSelectedRows = findNodeAll(toRaw(unref(flattedData)).concat(toRaw(unref(selectedRows))), (item) => rowKeys.includes(getRecordKey(item)), {
     ...
     */
    function getInvertRows(selectedRows, checked, flag) {
        if (flag == 'currentPage') {
...
function getTableBody(wrap) {
    return new Promise((resolve) => {
        (function fn() {
            const bodyEl = wrap.querySelector('.ant-table-wrapper .ant-table-body');
            if (bodyEl) {
                resolve(bodyEl);
            }
            else {
...
//# sourceMappingURL=useCustomSelection.jsx.map